...
# Rhinolabs theme colors (ANSI 256)
PRIMARY='\033[38;5;110m'      # #7FB4CA light blue
ACCENT='\033[38;5;179m'       # #E0C15A gold
SECONDARY='\033[38;5;146m'    # #A3B5D6 blue gray
MUTED='\033[38;5;242m'        # #5C6170 gray
SUCCESS='\033[38;5;150m'      # #B7CC85 green
ERROR='\033[38;5;174m'        # #CB7C94 pink/red
PURPLE='\033[38;5;183m'       # #C99AD6 purple
BOLD='\033[1m'
STRIKE='\033[9m'
NC='\033[0m'
...
get_mcp_servers() {
  ...
  if [ -f "$MCP_CACHE_FILE" ]; then
    CACHE_AGE=$(($(date +%s) - $(get_file_mtime "$MCP_CACHE_FILE")))
  ...
  if [ -n "$CURRENT_DIR" ] && [ -f ~/.claude.json ]; then
  ...
  if [ -z "$SERVERS" ] && [ -f ~/.claude.json ]; then
