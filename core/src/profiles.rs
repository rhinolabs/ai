...
impl Profiles {
    ...
    /// Load profiles.json from the plugin package (source of truth for skill assignments)
    fn load_plugin_profiles() -> Option<ProfilesConfig> {
        let plugin_dir = Paths::plugin_dir().ok()?;
        let plugin_profiles = plugin_dir.join("profiles.json");

        if !plugin_profiles.exists() {
            return None;
        }

        let content = fs::read_to_string(&plugin_profiles).ok()?;
...
"#,
    ...
    fn load_config() -> Result<ProfilesConfig> {
        ...
        if !path.exists() {
            // First time: seed from plugin's profiles.json if available,
            // otherwise create a default Main-Profile with no skills.
            let config = Self::load_plugin_profiles().unwrap_or_else(|| {
    ...
    pub fn install(
        ...
        for target in effective_targets {
            ...
            // Generate .gitignore for symlinked skills (Project profiles only)
            if profile.profile_type == ProfileType::Project {
                Self::generate_skills_gitignore(&skills_target, &profile.skills)?;
            }
        }
...
"#,
    ...
    /// Generate a `.gitignore` inside the skills directory that excludes symlinked skills.
    ///
    /// Only skills that are actual symlinks (machine-specific absolute paths) are ignored.
    /// Skills that were copied (fallback when symlinks aren't supported) remain tracked by git.
    ///
    /// The generated section is delimited by markers so it can be updated on re-runs
    /// without affecting user-added entries.
    fn generate_skills_gitignore(skills_dir: &Path, skill_ids: &[String]) -> Result<()> {
        // Detect which installed skills are symlinks
        let mut symlinked: Vec<&str> = Vec::new();
        for skill_id in skill_ids {
            let skill_path = skills_dir.join(skill_id);
            if let Ok(meta) = fs::symlink_metadata(&skill_path) {
                if meta.file_type().is_symlink() {
                    symlinked.push(skill_id);
                }
            }
        }
        ...
        // If no symlinks exist, remove auto-generated section (or skip entirely)
        if symlinked.is_empty() {
            if gitignore_path.exists() {
                let content = fs::read_to_string(&gitignore_path)?;
                let cleaned = Self::strip_auto_section(&content);
        ...
        // Preserve existing user entries (everything outside our auto-generated section)
        let user_entries = if gitignore_path.exists() {
            let content = fs::read_to_string(&gitignore_path)?;
            Self::strip_auto_section(&content)
        } else {
            String::new()
        };

        // Build new content
        let mut content = user_entries;

        // Ensure separation from user entries
        ...
        content
            .push_str("# Auto-generated by rhinolabs-ai — symlinked skills (machine-specific)\n");
        content.push_str("# Re-run `rlai profile install` to regenerate\n");
        symlinked.sort();
        for skill_id in &symlinked {
            content.push_str(skill_id);
            content.push('\n');
        }
        content.push_str("# End rhinolabs-ai\n");

        fs::write(&gitignore_path, content)?;
        Ok(())
    }

    /// Remove the auto-generated section from a `.gitignore` content string,
    /// preserving everything outside the markers.
    fn strip_auto_section(content: &str) -> String {
        let mut result = String::new();
        let mut in_auto_section = false;

        for line in content.lines() {
            if line.contains("Auto-generated by rhinolabs-ai") {
                in_auto_section = true;
                continue;
            }
            if in_auto_section && line.contains("End rhinolabs-ai") {
...
mod tests {
    ...
    #[test]
    fn test_create_main_profile_fallback_has_no_skills() {
        let _lock = ENV_MUTEX.lock().unwrap();
        let _env = TestEnv::new();

        // create_main_profile is a fallback when no plugin profiles.json exists.
        // It should NOT scan the filesystem — it creates an empty profile.
    ...
    fn test_load_config_seeds_from_plugin_profiles_json() {
        ...
        let plugin_config = ProfilesConfig {
            profiles: vec![Profile {
                ...
                profile_type: ProfileType::User,
                skills: vec![
                    "rhinolabs-security".to_string(),
                    "rhinolabs-architecture".to_string(),
                ],
                auto_invoke_rules: Vec::new(),
                instructions: None,
                generate_copilot: false,
        ...
        let plugin_dir = Paths::plugin_dir().unwrap();
        let plugin_profiles_path = plugin_dir.join("profiles.json");
        let content = serde_json::to_string_pretty(&plugin_config).unwrap();
        fs::write(&plugin_profiles_path, content).unwrap();

        // load_config should seed from plugin profiles.json
    ...
    #[test]
    fn test_load_config_preserves_empty_main_profile_skills() {
        let _lock = ENV_MUTEX.lock().unwrap();
        let env = TestEnv::new();

        // Create a profiles.json with an empty main profile
        ...
        // load_config should NOT auto-populate skills from filesystem
        let config = Profiles::load_config().unwrap();

        let main = config.profiles.iter().find(|p| p.id == "main").unwrap();
        assert!(main.skills.is_empty());
    }
    ...
    #[test]
    fn test_generate_skills_gitignore_creates_file_for_symlinks() {
        let _lock = ENV_MUTEX.lock().unwrap();
        let _env = TestEnv::new();
        ...
        let real_skill = skills_dir.join("my-custom-skill");
        fs::create_dir_all(&real_skill).unwrap();

        // Create a symlink
        let symlink_source = tmp.path().join("source-skill");
        fs::create_dir_all(&symlink_source).unwrap();
        #[cfg(unix)]
        std::os::unix::fs::symlink(&symlink_source, skills_dir.join("linked-skill")).unwrap();

        let skill_ids = vec!["my-custom-skill".to_string(), "linked-skill".to_string()];
        Profiles::generate_skills_gitignore(&skills_dir, &skill_ids).unwrap();

        let gitignore = fs::read_to_string(skills_dir.join(".gitignore")).unwrap();

        // Symlinked skill should be in .gitignore (no trailing slash — git treats symlinks as files)
        assert!(
            gitignore.contains("\nlinked-skill\n"),
            "Symlinked skill should be ignored"
        );
        // Real directory should NOT be in .gitignore
        assert!(
            !gitignore.contains("my-custom-skill"),
            "Non-symlinked skill should not be ignored"
        );
        // Should have markers
        assert!(gitignore.contains("Auto-generated by rhinolabs-ai"));
        assert!(gitignore.contains("End rhinolabs-ai"));
    }

    #[test]
    fn test_generate_skills_gitignore_preserves_user_entries() {
        let _lock = ENV_MUTEX.lock().unwrap();
        let _env = TestEnv::new();
        ...
        let gitignore_path = skills_dir.join(".gitignore");
        fs::write(&gitignore_path, "# My custom ignores\n*.tmp\n").unwrap();

        // Create a symlink
        let symlink_source = tmp.path().join("source-skill");
        fs::create_dir_all(&symlink_source).unwrap();
        #[cfg(unix)]
        std::os::unix::fs::symlink(&symlink_source, skills_dir.join("react-patterns")).unwrap();

        let skill_ids = vec!["react-patterns".to_string()];
        Profiles::generate_skills_gitignore(&skills_dir, &skill_ids).unwrap();

        let gitignore = fs::read_to_string(&gitignore_path).unwrap();

        // User entries preserved
        assert!(gitignore.contains("# My custom ignores"));
        assert!(gitignore.contains("*.tmp"));
        // Auto-generated entry added
        assert!(gitignore.contains("\nreact-patterns\n"));
    }

    #[test]
    fn test_generate_skills_gitignore_updates_on_rerun() {
        let _lock = ENV_MUTEX.lock().unwrap();
        let _env = TestEnv::new();
        ...
        #[cfg(unix)]
        {
            std::os::unix::fs::symlink(&source_a, skills_dir.join("skill-a")).unwrap();
            std::os::unix::fs::symlink(&source_b, skills_dir.join("skill-b")).unwrap();
        }

        // First run
        let ids = vec!["skill-a".to_string(), "skill-b".to_string()];
        Profiles::generate_skills_gitignore(&skills_dir, &ids).unwrap();

        // Now remove skill-b symlink and add skill-c
        fs::remove_file(skills_dir.join("skill-b")).unwrap();
        let source_c = tmp.path().join("source-c");
        fs::create_dir_all(&source_c).unwrap();
        #[cfg(unix)]
        std::os::unix::fs::symlink(&source_c, skills_dir.join("skill-c")).unwrap();

        // Second run with updated list
        let ids = vec!["skill-a".to_string(), "skill-c".to_string()];
        Profiles::generate_skills_gitignore(&skills_dir, &ids).unwrap();
        ...
        assert!(gitignore.contains("\nskill-a\n"));
        assert!(gitignore.contains("\nskill-c\n"));
        // skill-b should be gone (no longer symlinked)
        assert!(!gitignore.contains("skill-b"));
        // Should have exactly one auto-generated section
        assert_eq!(
            gitignore.matches("Auto-generated by rhinolabs-ai").count(),
            1
        );
    }

    #[test]
    fn test_generate_skills_gitignore_no_symlinks_removes_section() {
        let _lock = ENV_MUTEX.lock().unwrap();
        let _env = TestEnv::new();
        ...
        // Create .gitignore with user content + auto section
        let gitignore_path = skills_dir.join(".gitignore");
        fs::write(
            &gitignore_path,
            "# User stuff\n*.log\n# Auto-generated by rhinolabs-ai — symlinked skills (machine-specific)\n# Re-run `rlai profile install` to regenerate\nold-skill/\n# End rhinolabs-ai\n",
        )
        .unwrap();

        // Real directory only (no symlinks)
        let real_skill = skills_dir.join("my-skill");
        fs::create_dir_all(&real_skill).unwrap();

        let ids = vec!["my-skill".to_string()];
        Profiles::generate_skills_gitignore(&skills_dir, &ids).unwrap();
        ...
        assert!(!gitignore.contains("Auto-generated by rhinolabs-ai"));
        assert!(!gitignore.contains("old-skill"));
        // User content preserved
        assert!(gitignore.contains("# User stuff"));
        assert!(gitignore.contains("*.log"));
    }

    #[test]
    fn test_generate_skills_gitignore_no_symlinks_no_user_content_removes_file() {
        let _lock = ENV_MUTEX.lock().unwrap();
        let _env = TestEnv::new();
        ...
        // Create .gitignore with only auto section
        let gitignore_path = skills_dir.join(".gitignore");
        fs::write(
            &gitignore_path,
            "# Auto-generated by rhinolabs-ai — symlinked skills (machine-specific)\n# Re-run `rlai profile install` to regenerate\nold-skill/\n# End rhinolabs-ai\n",
        ...
        assert!(
            ...
            ".gitignore should be removed when only auto-section existed and no symlinks remain"
        );
    }

    #[test]
    fn test_strip_auto_section() {
        let input = "# User stuff\n*.log\n# Auto-generated by rhinolabs-ai — symlinked skills (machine-specific)\n# Re-run `rlai profile install` to regenerate\nskill-a/\nskill-b/\n# End rhinolabs-ai\n# More user stuff\n*.tmp\n";
        let result = Profiles::strip_auto_section(input);
